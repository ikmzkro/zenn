...
# マークルツリーとは何か
## マークルツリーの基本概念
マークルツリーは、[包含証明](https://zenn.dev/link/comments/75c3f508fb9744)の効率的なためにデザインされたデータ構造の一例です。軽量クライアントなど、完全なブロックチェーンデータを保持できない状況では、マークルツリーを使用してブロックヘッダのマークルルートフィールドを確認することで、トランザクションの整合性を検証し、取引の安全性を確保できます。
