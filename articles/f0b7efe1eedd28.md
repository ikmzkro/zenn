---
title: "【Merkle Tree】発行先アドレスと発行数を制御したERC721コントラクト"
emoji: "🌲"
...
# どんな機能？
今回は各ウォレットアドレスに対するトークン発行数を特定のホワイトリストで管理するスマートコントラクトを実装しました。それぞれのアドレスに対し、NFTの発行可能な数を明示したホワイトリストを作成し、このリストに基づいてNFTの発行を制御するためにマークルツリー(Merkle Tree)の技術を利用しました。
